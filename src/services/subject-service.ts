...
export const getLearnerRankingsBySubject = async (subjectId: string) => {
  // Fetch progress for the given subject and user, along with user name and completion status
  ...
  const learnerCompletionRates = progressData.reduce(
    (acc, { userId, isCompleted, user }) => {
      if (!acc[userId]) {
        acc[userId] = { completed: 0, total: 0, name: user.name };
      }
      acc[userId].total += 1;
      if (isCompleted) {
  ...
  const rankings = Object.keys(learnerCompletionRates).map((userId) => {
    const { completed, total, name } = learnerCompletionRates[userId];
    const completionRate = total > 0 ? completed / total : 0;
    return { userId, name, completionRate };
  });
